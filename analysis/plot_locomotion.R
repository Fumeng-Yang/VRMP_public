vr_head <- subset(vr, device == "Camera")

FONT_SIZE <- 8

locomation_x <- ggplot(data = vr_head)  %>%
  ...
  + ylab("Participants")  %>%
  + xlab("Navigation - x (meter)")  %>%
  + scale_y_reverse(expand = c(0.05, 0.05), breaks=seq(13,1,by=-1)) %>%
  # + coord_flip() %>%
  # + facet_grid(id)  %>%
  + scale_x_continuous(expand = c(0.05, 0.05), breaks=seq(-3,3.5,by=1), limits=c(-3,3.5)) %>%
  #+ scale_y_continuous(expand = c(0.05, 0.05), breaks=seq(13,1,by=-1)) %>%
  + guides(fill=FALSE)
locomation_x

locomation_y <- ggplot(data = vr_head)  %>%
  + theme(axis.line = element_line(colour = "gray", size = AXIS_LINE_SIZE, linetype = "solid"), 
          axis.line.y = element_blank(),
          axis.text = element_text(size = FONT_SIZE, color = darkgray),
          axis.text.x = element_text(vjust=0.5, hjust=0.5, face="plain"),
  ...
  + ylab("Participants")  %>%
  + xlab("Navigation - y (meter)")  %>%
  # + coord_flip() %>%
  # + facet_grid(id)  %>%
  + scale_x_continuous(expand = c(0.05, 0.05), breaks=seq(-4,4,by=1)) %>%
  + scale_y_reverse(expand = c(0.05, 0.05), breaks=seq(13,1,by=-1)) %>%
  + guides(fill=FALSE)
locomation_y

locomation_z <- ggplot(data = vr_head)  %>%
  + theme(axis.line = element_line(colour = "gray", size = AXIS_LINE_SIZE, linetype = "solid"), 
          axis.line.y = element_blank(),
          axis.text = element_text(size = FONT_SIZE, color = darkgray),
          axis.text.x = element_text(vjust=0.5, hjust=0.5, face="plain"),
  ...
  + ylab("Participants")  %>%
  + xlab("Navigation - z (meter)")  %>%
  # + coord_flip() %>%
  # + facet_grid(id)  %>%
  + scale_x_continuous(expand = c(0.05, 0.05), breaks=seq(-6,6,by=2)) %>%
  + scale_y_reverse(expand = c(0.05, 0.05), breaks=seq(13,1,by=-1)) %>%
  + guides(fill=FALSE)
locomation_z

ggsave(file = 'img/locomation-x.png', plot = locomation_x, width = 5.2, height = 2, units = "in")
ggsave(file = 'img/locomation-y.png', plot = locomation_y, width = 5.2, height = 2, units = "in")
ggsave(file = 'img/locomation-z.png', plot = locomation_z, width = 5.2, height = 2, units = "in")
